...
import { api, type BlockedDay, type Order } from '../api'
...
export default function Calendar() {
  ...
  const load = useCallback(async () => {
    const [b, o] = await Promise.all([api.get<BlockedDay[]>('/calendar/blocked'), api.get<Order[]>('/orders')])
    setBlocked(b)
    setOrders(o)
  }, [])
          ...
          {Array.from({ length: daysInMonth }).map((_, i) => {
            ...
            return (
              <button
                ...
                } ${selected === key ? 'ring-2 ring-white' : ''}`}
              >
                {i + 1}
